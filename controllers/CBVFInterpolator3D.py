...
class CBVFInterpolator:
    ...
    def _create_interpolator(self) -> Callable:
        ...
        x3_range = (self.grid.domain.lo[2], self.grid.domain.hi[2])
        ...
        def _interpolator(state, time):
            ...
            x3_idx = (state[2] - x3_range[0]) / (x3_range[1] - x3_range[0]) * (data.shape[3] - 1)

            coordinates = jnp.array([[t_idx], [x1_idx], [x2_idx], [x3_idx]])
    ...
    def interpolate_spatial_gradient(self, state: jnp.ndarray, time: float) -> jnp.ndarray:
        """Compute spatial gradient [∂V/∂x1, ∂V/∂x2, ∂V/∂x3] at given state and time."""
    ...
    def get_value_and_gradient(self, state: jnp.ndarray, time: float) -> Tuple[float, jnp.ndarray, float]:
        ...
        Returns:
            ...
            spatial_grad: Spatial gradient [∂V/∂x1, ∂V/∂x2, ∂V/∂x3]
    ...
    def compute_safe_entry_gradients_efficient(self, states: jnp.ndarray) -> Tuple[
        ...
        Args:
            states: Array of states to analyze, shape (n_states, 3)
        ...
        Returns:
            ...
            spatial_grads: Spatial gradients at entry, shape (n_states, 3)
        ...
        def state_to_indices(state):
            ...
            x3_idx = (state[2] - x3_range[0]) / (x3_range[1] - x3_range[0]) * (self.cbvf_values.shape[3] - 1)
            return jnp.array([x1_idx, x2_idx, x3_idx])
        ...
        def find_safe_entry_efficient(state):
            ...
            def get_value_at_time_idx(t_idx):
                coords = jnp.array([[t_idx], [spatial_coords[0]], [spatial_coords[1]], [spatial_coords[2]]])
